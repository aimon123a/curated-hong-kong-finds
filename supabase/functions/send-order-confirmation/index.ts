// Standalone Edge Function — 下單確認電郵
// Paste this entire file into Supabase Dashboard → Edge Functions → send-order-confirmation
...
const buildOrderConfirmationEmail = (o: OrderEmailData) => {
  const body = `
...
async function updateEmailStatus(
  ...
  const patch: Record<string, unknown> = {
    ...
    confirmation_email_error: status === "sent" ? null : error,
  };
  if (status === "sent") patch.confirmation_email_sent_at = new Date().toISOString();
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.to || !body.orderNumber || !body.customerName) {
      throw new Error("Missing required fields (to, orderNumber, customerName)");
    }
